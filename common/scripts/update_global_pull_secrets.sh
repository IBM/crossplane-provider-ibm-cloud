...
if [[ -z "$(echo "${new_pull_secret}" | grep 'docker-na-public.artifactory.swg-devops')" ]]; then
    registry_pull_secret="\"docker-na-public.artifactory.swg-devops.com\":{\"auth\":\"${artifactory_secret}\"}"
